...
function MainLayout() {
  ...
  const hasStock = (spec: string) => {
    let accumulator = 0;
    let sum = Object.values(specs[inch][spec]).reduce((accumulator: any, currentValue: any) => {
      ...
      <View style={styles.display}>
        ...
        <ScrollView>
          <View style={styles.display_option}>
            {specs[inch] && Object.keys(specs[inch]).sort().map((spec) => (
              <Chip
                key={spec}
                mode="outlined"
                style={hasStock(spec)}
            ...
            {specs[inch] ? (
              Object.keys(specs[inch]).map((item, idx) => (
                <View key={idx} style={styles.display_item}>
                  <Text style={itemSelected(item)}>{removeLastThreeChars(item)}</Text>
                  {specs[inch][item] ? (
                    Object.keys(specs[inch][item]).sort().map((loc, idx) => (
                      <View key={idx} style={styles.display_detail}>
                        <Text>{loc} - {specs[inch][item][loc]}</Text>
                      </View>
                    ))
                  ) : (<Text>Loading</Text>)}
                </View>
